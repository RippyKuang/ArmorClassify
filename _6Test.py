...
if __name__ == "__main__":
    ...
    Data_Folder = "Dataset"
    # 导入训练集与测试集
    TrainDataset, TrainDataLoader, ValDataset, ValDataLoader = PipeDatasetLoader(Data_Folder, 1,
                                                                                 TrainTransform=TrainImg_Transform,
                                                                                 ValTransform=ValImg_Transform)
    ...
    for num, model in enumerate(Models):
        ...
        # 绘制单个模型的混淆矩阵
        print(Matrix)
        AccuracyMatrix = plot_confusion_matrix(Matrix, classes=Classes, normalize=True,
                                               title='confusion matrix' + str(num))
