...
def Traverse_Folder(Father_Folder,Init_Path,Lists,Target_Suffix) -> list:
    ...
    for child1 in Father_Folder:
        Child1_Path = os.path.join(Init_Path, child1) # concatenate two string as an entire path
        Suffix = os.path.splitext(Child1_Path)[1] # 分离文件名与扩展名 s
...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.xticks(tick_marks, classes, rotation=45) #replace the default index as assigned classes index
